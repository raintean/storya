...
const NativeAudioStreamController = Hls.DefaultConfig.audioStreamController
if (NativeAudioStreamController === undefined) {
  throw new Error('当前 hls.js 构建不包含 AudioStreamController')
}

export class ParallelAudioStreamController extends NativeAudioStreamController {
  ...
  private requireSegmentLoader(): ParallelSegmentLoader {
    ...
    if (loader === undefined) {
      throw new Error(
        'ParallelAudioStreamController 必须与同一个 ParallelSegmentLoader.fLoader 配合使用',
      )
    }
    return loader
  }
